...
import {
  ...
  EMPTY_RECIBO_FORM, MOCK_RECIBOS, MOCK_MAYOR_30, MOCK_SIN_CANCELAR,
  MOCK_FINANCIERO, MOCK_SALARIOS, MOCK_RELACION, MOCK_IMPRIMIR
} from './types';
...
const Excedentes: React.FC = () => {
  ...
  const [mayor30, setMayor30] = useState<ExcedenteMayor30[]>(MOCK_MAYOR_30);
  ...
  const [sinCancelar, setSinCancelar] = useState<ExcedenteSinCancelar[]>(MOCK_SIN_CANCELAR);
  ...
  const fetchTabData = async (tab: string) => {
    ...
    try {
      if (tab === 'Recibos de Pago') {
        setRecibos(MOCK_RECIBOS);
      } else if (tab === 'Excedentes mayor a 30 días') {
        setMayor30(MOCK_MAYOR_30);
      } else if (tab === 'Excedentes sin cancelar') {
        setSinCancelar(MOCK_SIN_CANCELAR);
      }
    } catch (err: any) {
      setErrorStatus('Error al cargar los datos');
    } finally {
      setLoading(false);
      setCurrentPage(1);
    }
  ...
  const recibosTotales = useMemo(() => {
    const parse = (s: any) => {
      const val = typeof s === 'string' ? s : String(s || 0);
  ...
  const handleSaveRecibo = async () => {
    ...
    try {
      if (isNewMode) {
        const newRecibo: ReciboPago = {
          ...reciboForm,
          id: Math.max(0, ...recibos.map(r => r.id)) + 1,
          numeroRecibo: `REC-${String(recibos.length + 1).padStart(3, '0')}`,
          estado: 'Pendiente'
        };
        setRecibos(arr => [...arr, newRecibo]);
  ...
  const handleImprimirBuscar = async () => {
    ...
    try {
      setImprimirItems(MOCK_IMPRIMIR);
  ...
  const handleRelacionGenerar = async () => {
    ...
    try {
      setRelacionItems(MOCK_RELACION);
  ...
  const handleFinancieroGenerar = async () => {
    ...
    await new Promise(resolve => setTimeout(resolve, 600));
    try {
      setFinancieroTotales(MOCK_FINANCIERO);
    } catch (e) { console.error(e); }
    setLoading(false);
  };
  ...
  const handleSalariosGenerar = async () => {
    ...
    await new Promise(resolve => setTimeout(resolve, 700));
    try {
      setSalariosItems(MOCK_SALARIOS);
